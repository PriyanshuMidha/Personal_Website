...
- Experience CMS array fields such as `responsibilities`, `achievements`, and `techStack` now accept multiple values entered as either comma-separated text or one-per-line input, with field-specific placeholders and helper copy in the editor.
